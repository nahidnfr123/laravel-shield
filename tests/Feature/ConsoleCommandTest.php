...
class ConsoleCommandTest extends TestCase {
    ...
    public function test_roles_with_privileges_command_outputs_privileges(): void {
        ...
        $exitCode = Artisan::call('shield:roles-with-privileges');
    ...
    public function test_delete_privilege_command_deletes_privilege(): void {
        ...
        $this->artisan('shield:delete-privilege', [
    ...
    public function test_delete_privilege_command_prompts_for_identifier(): void {
        ...
        $this->artisan('shield:delete-privilege', ['--force' => true])
    ...
    public function test_attach_and_detach_privilege_commands(): void {
        ...
        $this->artisan('shield:attach-privilege', [
            'privilege' => $privilege->slug,
            'role' => $role->slug,
        ...
        $this->artisan('shield:detach-privilege', [
            'privilege' => $privilege->slug,
            'role' => $role->slug,
    ...
    public function test_role_users_command_lists_users_for_given_role(): void {
        ...
        $exitCode = Artisan::call('shield:role-users', ['role' => $role->slug]);
        $output = Artisan::output();
    ...
    public function test_logout_command_revokes_single_token(): void {
        ...
        $this->artisan('shield:logout', ['token' => $token])
    ...
    public function test_quick_token_command_generates_token_by_user_id(): void {
        ...
        $this->artisan('shield:quick-token', ['user' => (string) $user->id])
            ->expectsOutputToContain('Token:')
            ->assertExitCode(0);
    ...
    public function test_quick_token_command_blocks_suspended_users(): void {
        ...
        $this->artisan('shield:quick-token', [
            'user' => (string) $user->id,
    ...
    public function test_update_user_command_updates_fields(): void {
        ...
        $this->artisan('shield:update-user', [
            '--user' => (string) $user->id,
            '--name' => 'Updated User',
    ...
    public function test_suspend_user_command_sets_and_lifts_suspension(): void {
        ...
        $this->artisan('shield:suspend-user', [
            '--user' => (string) $user->id,
    ...
    public function test_unsuspend_user_command_lifts_suspension(): void {
        ...
        $this->artisan('shield:unsuspend-user', [
            '--user' => (string) $user->id,
            '--force' => true,
        ])->expectsOutputToContain('no longer suspended')
            ->assertExitCode(0);
    ...
    public function test_suspended_users_command_lists_records(): void {
        ...
        $exitCode = Artisan::call('shield:suspended-users');
        $output = Artisan::output();
    ...
    public function test_users_command_marks_suspended_users(): void {
        ...
        $exitCode = Artisan::call('shield:users');
        $output = Artisan::output();
    ...
    public function test_update_role_command_updates_name_and_slug(): void {
        ...
        $this->assertDatabaseHas(config('shield.tables.roles', 'roles'), [
            'id' => $role->id,
            'name' => 'Content Editor',
            'slug' => 'content-editor',
        ]);
    ...
    public function test_update_privilege_command_updates_record(): void {
        ...
        $this->artisan('shield:update-privilege', [
            '--privilege' => $privilege->slug,
            '--name' => 'Docs Audit',
            '--slug' => 'docs.audit',
        ...
        $this->assertDatabaseHas(config('shield.tables.privileges', 'privileges'), [
            'id' => $privilege->id,
            'name' => 'Docs Audit',
            'slug' => 'docs.audit',
            'description' => 'Audit documentation',
    ...
    public function test_delete_user_role_command_detaches_role(): void {
        ...
        $this->artisan('shield:delete-user-role', [
            '--user' => $user->email,
            '--role' => $role->slug,
    ...
    public function test_user_roles_command_lists_roles(): void {
        ...
        $exitCode = Artisan::call('shield:user-roles', ['user' => $user->email]);
        $output = Artisan::output();
    ...
    public function test_user_privileges_command_lists_privileges(): void {
        ...
        $exitCode = Artisan::call('shield:user-privileges', ['user' => $user->email]);
        $output = Artisan::output();
    ...
    public function test_logout_all_command_revokes_every_token(): void {
        ...
        $this->artisan('shield:logout-all', ['--user' => $user->email, '--force' => true])
    ...
    public function test_logout_all_users_command_revokes_tokens_for_everyone(): void {
        ...
        $this->artisan('shield:logout-all-users', ['--force' => true])
    ...
    public function test_version_command_shows_value_from_config(): void {
        ...
        $this->artisan('shield:version')
            ->expectsOutput('Shield v1.4.0-test')
            ->assertExitCode(0);
    }
    ...
    public function test_seed_command_restores_defaults(): void {
        ...
        $this->artisan('shield:seed', ['--force' => true])
            ->assertExitCode(0);

        $this->assertDatabaseMissing($tempUser->getTable(), ['email' => 'temp@example.com']);
        $this->assertDatabaseMissing(config('shield.tables.roles'), ['slug' => 'temp-role']);
        $this->assertDatabaseHas('users', ['email' => 'admin@shield.project']);
        $this->assertSame(1, $userClass::count());
        $this->assertSame(6, Role::count());
        $this->assertSame(1, DB::table(config('shield.tables.pivot'))->count());
    ...
    public function test_publish_config_command_exports_file(): void {
        ...
        $this->artisan('shield:publish-config', ['--force' => true])
            ->assertExitCode(0);
...
PHP;
        ...
        try {
            $this->artisan('shield:prepare-user-model', ['--path' => $path])
            ...
            $this->assertStringContainsString('use NahidFerdous\\Shield\\Concerns\\HasShieldRoles;', $updated);
            $this->assertStringContainsString('use HasApiTokens, HasShieldRoles;', $updated);
